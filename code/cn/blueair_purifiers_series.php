...
<div class="bodyleftContainer"><h1><?=$get_serie['category_name'];?></h1><?=$get_serie['description_cn'];?><br /><br />
<? $sqlmodel=mysql_query("select * from tbl_products where cateid='".$get_serie['cate_id']."' and published=1 and lang = 2 order by id");
...
<?php if ($getmodel['image']) {?><img src="../uploads/<?=$getmodel['image'];?>" width="95" height="105" align="right" class="img"/><?php } ?><h1><?=$getmodel['product_name'];?></h1>
<p><?=substr(strip_tags($getmodel['description']),0,150);?>...<a href="blueair_purifiers_product.php?serieid=<?=$getmodel['cateid'];?>&product=<?=$getmodel['id'];?>">详细信息</a><br /><br /><strong>市场价</strong>:- <?=$getmodel['regular_price'];?>元<br /><strong style="color:#FF0000;">优惠价</strong>:- <? if($getmodel['our_price']>0){ echo $getmodel['our_price'];}else{ echo "XXX";}?>元</p>
</div>
<? $p++;}}?>
</div>
...
<? $sql_model=mysql_query("select * from tbl_products where cateid='".$series['cate_id']."' and lang = 2 and published=1 order by id");
